...
export default function MarqueeSection() {
  ...
  // Smooth out the scroll velocity for the "Kinetic Flow" and "Quantum Pulse" effects
  ...
  // "Vortex" Rotation Effect (spins slightly based on scroll depth)
  const rotateVortex = useTransform(scrollYProgress, [0, 1], [-5, 5]);

  const opacity = useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [0, 1, 1, 0]);

  // Duplicate arrays to ensure seamless loop
  ...
  return (
    <motion.section 
      ...
      className="relative w-full h-[50vh] sm:h-[80vh] flex flex-col justify-center overflow-hidden bg-transparent perspective-1000"
    >
      {/* "Aura" & "Spectral" glowing background effects */}
      ...
      >
        ...
        {/* Top Row - Scrolling Left (Kinetic Flow) */}
        ...
        {/* Bottom Row - Scrolling Right (Prism & Obsidian effects via styling) */}
        <div className="flex w-max relative">
          <motion.div 
            animate={{ x: ["-50%", "0%"] }}
            transition={{ duration: 40, ease: "linear", repeat: Infinity }}
